...
export function getProjectById(projectId: string): Promise<Project>{
  return axiosInstance.get(`api/project/${projectId}`).then(res => res.data);
}
...
export function deleteProject(projectId: string): Promise<Project> {
  return axiosInstance.delete(`api/project/${projectId}`).then(res => res.data);
}
export function inviteCoworker(projectId: string, data: {userId: string}): Promise<Project> {
  return axiosInstance.post(`api/project/${projectId}/coworker/invite`, data).then(res => res.data);
}
export function removeCoworker(projectId: string, userId: string): Promise<Project> {
  return axiosInstance.delete(`api/project/${projectId}/coworker/${userId}`).then(res => res.data);
}
export function joinCoworker(projectId: string): Promise<Project> {
  return axiosInstance.post(`api/project/${projectId}/coworker/join`).then(res => res.data);
}
export function rejectCoworker(projectId: string): Promise<Project> {
  return axiosInstance.post(`api/project/${projectId}/coworker/reject`).then(res => res.data);
}
export function getInvitedProjects(): Promise<Project[]> {
  return axiosInstance.get("api/project/invitedProjects").then(res => res.data);
}